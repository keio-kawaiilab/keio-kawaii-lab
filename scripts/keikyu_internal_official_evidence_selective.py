...
def load_context(
    ...
    fragments = [
        row for row in payload.get('fragments') or []
...
def needed_services(rows: list[tuple[dict[str, Any], list[dict[str, Any]]]]) -> set[str]:
    return {
        ...
        if (service := target.service_of(source))
    }
...
def needed_station_times(
    ...
    """Return exact station/minute keys that can participate in current ambiguities.

    This is only a performance filter. It never establishes train identity: the
    production generator still requires two exact points from one official
    printed column plus singleton fragment matches on both sides.
    """
    ...
    with pdfplumber.open(io.BytesIO(content)) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            for row in target.parser.rows(words):
                ...
                suffixes = {
                    suffix
                    for label, suffix in label_suffixes
...
def main() -> int:
    try:
        ...
        # Fail safe: retain the original full parser if prerequisite diagnostics
        # are missing or malformed. Never infer identity from incomplete context.
    ...
    print('KEIKYU_MAINLINE_EXACT_KEYS_NEEDED', {
        service: len(keys) for service, keys in sorted(needed.items())
    })
