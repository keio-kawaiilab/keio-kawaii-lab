...
class OfficialXSpecialEventTests(unittest.TestCase):
    ...
    def test_birthday_wording_is_tracked_as_solo_live(self):
        self.assertEqual(s.special_category("SWEET STEADY 庄司なぎさ BIRTHDAY LIVE 2026"), "solo-live")
        self.assertEqual(s.special_category("庄司なぎさ 生誕祭 2026"), "solo-live")
        self.assertEqual(s.special_category("庄司なぎさ 生誕ライブ"), "solo-live")
        self.assertEqual(s.special_category("庄司なぎさ BIRTHDAY EVENT"), "solo-live")

    def test_profile_parses_birthday_live(self):
        html = '''
        <html><body>
        <article itemtype="https://schema.org/SocialMediaPosting" itemid="https://x.com/i/status/444">
          <meta itemprop="url" content="https://x.com/SWEET_STEADY/status/444">
          <meta itemprop="description" content="SWEET STEADY 庄司なぎさ BIRTHDAY LIVE 2026\n📅 2026/10/26\n📍SGCホール有明\nOPEN 17:30 / START 19:00">
        </article>
        </body></html>
        '''
        events = s.parse_profile("SWEET STEADY", "SWEET_STEADY", html, date(2026, 8, 31))
        self.assertEqual(len(events), 1)
        event = events[0]
