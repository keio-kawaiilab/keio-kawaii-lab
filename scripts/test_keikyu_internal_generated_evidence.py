...
def entry(**changes) -> dict:
    base = {
        ...
        'sourceMatches': ['m1'],
        'targetMatches': ['a1'],
        'evidence': ['operator-official-connection-timetable', target.MARKER],
...
def graph(*, verified: bool = True) -> dict:
    rows = []
    if verified:
        rows.append({'toRailway': target.AIRPORT, 'boundaryId': target.BOUNDARY_ID})
...
class ConsumerTests(unittest.TestCase):
    ...
    def test_wrong_pair_fails_closed(self) -> None:
        edges, unresolved = self.apply(payload(entry(toRailway='odpt.Railway:Keikyu.Kurihama')))
        self.assertEqual([], edges)
