...
CANONICAL_OFFER_JS = (
    "function expandSpecialOccurrences(raw){var out=[];(raw||[]).forEach(function(e){"
    "if(!e||e.displayByOccurrence!==true||!e.occurrenceDetails||!Array.isArray(e.schedule)){out.push(e);return}"
    "e.schedule.forEach(function(o,i){if(!o||!o.date)return;var day=String(o.date).slice(0,10),d=e.occurrenceDetails[day]||{},x=Object.assign({},e,o,d),us=[];"
    "x.id=String(e.id||'special-event')+'--'+day+(i?'-'+i:'');x.seriesId=e.id||null;x.eventDate=day;x.eventDates=[day];x.eventEndDate=day;x.eventCount=1;x.schedule=[Object.assign({},o)];"
    "if(o.venue)x.venue=o.venue;if(d.sourceUrl){x.url=d.sourceUrl;[d.sourceUrl].concat(e.urls||[]).forEach(function(u){if(u&&us.indexOf(u)<0)us.push(u)});x.urls=us}"
    "delete x.sourceUrl;delete x.occurrenceDetails;delete x.displayByOccurrence;out.push(x)})});return out}"
    "function expandCanonicalOffers(raw){raw=expandSpecialOccurrences(raw);var out=[];(raw||[]).forEach(function(e){"
...
def canonicalize_public_data() -> dict:
    ...
    normalized, normalization_report = normalize_payload(payload)
    corrected, occurrence_report = apply_known_occurrence_corrections(normalized)
    errors = validate(corrected)
    if errors:
        raise RuntimeError("canonical special-event normalization failed: " + "; ".join(errors))
    DATA.write_text(json.dumps(corrected, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return {
        "normalization": normalization_report,
        "occurrenceCorrections": occurrence_report,
    }


def build_public_snapshot() -> None:
    """Render marked multi-date entities as date-scoped cards without splitting canonical data on disk."""
    canonical_text = DATA.read_text(encoding="utf-8")
    payload = json.loads(canonical_text)
    display_payload = dict(payload)
    display_payload["events"] = expand_occurrence_views(payload.get("events") or [])
    DATA.write_text(json.dumps(display_payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    try:
        ...
        DATA.write_text(canonical_text, encoding="utf-8")
...
def install_offer_adapter(page: str) -> str:
    current_start = page.find("function expandSpecialOccurrences(raw)")
    legacy_start = page.find("function expandCanonicalOffers(raw)")
    if current_start >= 0:
        end = page.find("function prepare(raw)", current_start)
        if end < 0:
            raise RuntimeError("could not locate prepare() after occurrence adapter")
        page = page[:current_start] + CANONICAL_OFFER_JS + page[end:]
    elif legacy_start >= 0:
        end = page.find("function prepare(raw)", legacy_start)
        if end < 0:
            raise RuntimeError("could not locate prepare() after canonical offer adapter")
        page = page[:legacy_start] + CANONICAL_OFFER_JS + page[end:]
    else:
...
def main() -> int:
    ...
    required = (
        ...
        "if(day&&time)return [group,day,'time',time].join('|')",
        "function expandSpecialOccurrences(raw)",
        "e.displayByOccurrence!==true",
        "function expandCanonicalOffers(raw)",
        "raw=expandSpecialOccurrences(raw)",
