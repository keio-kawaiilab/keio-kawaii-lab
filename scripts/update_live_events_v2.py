...
def build_payload(existing: dict, fresh_by_id: dict[str, dict], pending: list[dict], failures: list[dict], today: date) -> dict:
    ...
    # Application deadline never removes a future show. Only the final show date does.
    ...
    # outside the first pages scanned by the crawler.
    retained: list[dict] = []
    ...
    return {
        ...
        "source": "KAWAII LAB.各グループ公式サイトの公開INFORMATION（本日以降の公演のみ）",
        "events": result,
