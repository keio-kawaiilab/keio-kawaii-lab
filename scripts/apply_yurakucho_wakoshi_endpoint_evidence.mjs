...
for(const item of evidence){
  ...
  const record={id:identityKey,identityKey,identityType:'odpt-explicit-boundary-endpoint',status:'verified',corridor:'line8',sourceOperator:row.sourceOperator||(item.sourceSide==='tobu'?'tobu':'tokyometro'),sourceSide:item.sourceSide,sourceTimetableId:row.timetableId,sourceTrainId:row.trainId||'',calendars:row.calendars||[],trainType:row.trainType||'',trainNumber:row.trainNumber||'',fromRailway:item.fromRailway,toRailway:item.toRailway,routeRailways:[item.fromRailway,item.toRailway],transitions:[{fromRailway:item.fromRailway,toRailway:item.toRailway,boundaryStation:item.boundaryEndpoint}],classification:'through',canonicalBoundaryId:BOUNDARY,boundaryEndpoint:item.boundaryEndpoint,externalOriginStations:[],externalDestinationStations:item.externalStations,exactEndpointRole:item.endpointRole,evidence:'same-odpt-train-timetable+explicit-other-operator-destination+exact-wakoshi-boundary-endpoint',matchPolicy:{sameTrainTimetableRecordRequired:true,exactBoundaryEndpointRequired:true,explicitOtherOperatorStationRequired:true,twoSidedSourceRecordsAllowed:true,destinationAloneMayEstablishIdentity:false,originAloneMayEstablishIdentity:false,timeProximityAloneMayEstablishIdentity:false,trainNumberAloneMayEstablishIdentity:false},runtimeRule:{requiredMatch:['identityKey','fromRailway','toRailway']}};
  if(!recordMap.has(identityKey)){recordMap.set(identityKey,record);added++;}
}
trips.version=Math.max(Number(trips.version)||0,8);trips.policy={...(trips.policy||{}),runtimeInference:false,timeGapMayEstablishTrainIdentity:false,trainNumberMayEstablishTrainIdentity:false,genericBoundaryChaining:false};trips.records=[...recordMap.values()].sort((a,b)=>String(a.id||'').localeCompare(String(b.id||'')));trips.yurakuchoWakoshiEndpointEvidence={sourceFile:identityFile,boundaryId:BOUNDARY,evidenceRecords:evidence.length,addedRecords:added,directions:counts,policy:{sameTrainTimetableRecordRequired:true,exactBoundaryEndpointRequired:true,twoSidedSourceRecordsAllowed:true,destinationAloneMayEstablishIdentity:false,timeProximityAloneMayEstablishIdentity:false,trainNumberAloneMayEstablishIdentity:false}};writeJson(tripsFile,trips);
coverage.version=Math.max(Number(coverage.version)||0,8);coverage.summary={...(coverage.summary||{}),throughRecords:trips.records.length,odptExplicitBoundaryEndpointThroughRecords:trips.records.filter(row=>row.identityType==='odpt-explicit-boundary-endpoint').length};coverage.yurakuchoWakoshiEndpointEvidence={sourceFile:identityFile,boundaryId:BOUNDARY,evidenceRecords:evidence.length,addedRecords:added,directions:counts};writeJson(coverageFile,coverage);
