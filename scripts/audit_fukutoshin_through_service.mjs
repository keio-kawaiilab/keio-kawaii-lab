...
function recordRailways(row){
  ...
  return [String(row.fromRailway||''),String(row.toRailway||'')];
}
function exactType(row){return String(row.identityType||row.evidenceType||'');}
function sameTriple(value,patterns){
...
for(const spec of PAIRS){
  ...
  const official=officialPages.get(spec.id).size;
  const exact=generated.get(spec.id).size;
  const authoritative=odpt+official;
  summary.boundaries[spec.id]={label:spec.label,authoritativeOdptLinks:odpt,officialSingleTrainPageEvidence:official,authoritativeExactEvidence:authoritative,generatedExactThroughRecords:exact};
