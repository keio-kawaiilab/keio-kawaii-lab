...
def occurrence_rows(event: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if isinstance(event.get("schedule"), list) and event["schedule"]:
        for item in event["schedule"]:
            if not isinstance(item, dict) or not item.get("venue"):
                continue
            day = parse_day(item.get("date") or event.get("eventDate"))
            if day:
                rows.append({
                    "date": day,
    ...
    if not venue or re.search(r"オンライン|複数会場|会場未定|その他\s", venue):
        return rows
    event_days = event.get("eventDates") if isinstance(event.get("eventDates"), list) else [event.get("eventDate")]
...
def prefecture_from_text(text: str) -> str:
    match = PREFECTURE_RE.search(str(text or ""))
    if match:
...
def geocode(session: requests.Session, venue_name: str, venue_record: dict[str, Any] | None, cache: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for query in queries:
        ...
        seen.add(query)
        try:
            payload = get_json(session, GSI_GEOCODE_URL, {"q": query})
        except Exception:
            continue
...
def class10_for_location(office_code: str, location_text: str, area_map: dict[str, Any]) -> str:
    class20s = area_map.get("class20s") or {}
    class15s = area_map.get("class15s") or {}
    class10s = area_map.get("class10s") or {}
    candidates: list[tuple[int, str]] = []
    for area20 in class20s.values():
...
def class10_name(class10_code: str, area_map: dict[str, Any]) -> str:
...
def weather_label(code: str, raw: str = "") -> str:
    ...
    if raw:
        return raw
    return WEATHER_CODE_LABELS.get(str(code), {"1": "晴れ", "2": "くもり", "3": "雨", "4": "雪"}.get(str(code or "")[:1], "天気予報"))
...
def first_area(series: dict[str, Any]) -> dict[str, Any] | None:
    areas = series.get("areas", []) if isinstance(series, dict) else []
    return areas[0] if areas else None


def date_index(series: dict[str, Any], day: str) -> int:
...
def parse_temp_values(series: dict[str, Any], area: dict[str, Any] | None, day: str) -> tuple[float | None, float | None]:
    if not area:
        return None, None
    values: list[float] = []
    temps = area.get("temps") or []
    for idx, ts in enumerate(series.get("timeDefines") or []):
...
def broad_forecast(payload: list[Any], day: str, office_code: str, class10_code: str, point_code: str, start_time: str) -> dict[str, Any] | None:
    if not isinstance(payload, list) or not payload:
        return None
    start_minutes = parse_minutes(start_time)

    short = payload[0] if isinstance(payload[0], dict) else {}
    short_series = short.get("timeSeries") or []
    if short_series:
        weather_series = short_series[0]
        weather_area = find_area(weather_series, class10_code)
        wi = date_index(weather_series, day)
        if weather_area is not None and wi >= 0:
            ...
            result: dict[str, Any] = {
                "label": weather_label(code, str(texts[wi]) if wi < len(texts) else ""),
                "code": code,
                "forecastAreaName": str((weather_area.get("area") or {}).get("name") or ""),
                "max": None,
                "min": None,
                "pop": None,
                "popLabel": "",
            ...
            if len(short_series) > 1:
                ...
                if pop_area:
                    ...
                    for idx, ts in enumerate(pop_series.get("timeDefines") or []):
                        if str(ts)[:10] != day or idx >= len(values) or values[idx] in (None, ""):
                            continue
                        dt = datetime.fromisoformat(str(ts))
                        choices.append((dt.hour * 60 + dt.minute, int(values[idx]), str(values[idx])))
                    if choices:
            ...
            if len(short_series) > 2:
                ...
                temp_area = find_area(temp_series, point_code) or first_area(temp_series)
                result["min"], result["max"] = parse_temp_values(temp_series, temp_area, day)
            return result
    ...
    # 週間予報は東京地方のような一次細分区域の場合と、埼玉県・千葉県のような府県単位の場合がある。
    weather_area = find_area(weather_series, class10_code) or find_area(weather_series, office_code)
    if weather_area is None:
        weather_area = first_area(weather_series)
    wi = date_index(weather_series, day)
    ...
    if len(weekly_series) > 1:
        ...
        temp_area = find_area(temp_series, point_code) or first_area(temp_series)
        ti = date_index(temp_series, day)
        if temp_area is not None and ti >= 0:
            mins, maxs = temp_area.get("tempsMin") or [], temp_area.get("tempsMax") or []
            try:
...
def choose_mesh_target(targets: list[dict[str, Any]], day: str, start_time: str) -> dict[str, Any] | None:
    ...
    desired = desired if desired is not None else 15 * 60
    ...
    for item in targets:
        ...
        if valid.date().isoformat() == day:
            candidates.append((abs(valid.hour * 60 + valid.minute - desired), item))
...
def closest_palette(rgb: tuple[int, int, int], palette: dict[str, tuple[int, int, int]]) -> str:
    return min(palette, key=lambda label: sum((rgb[i] - palette[label][i]) ** 2 for i in range(3)))


def sample_tile(session: requests.Session, target: dict[str, Any], element: str, lon: float, lat: float, palette: dict[str, tuple[int, int, int]]) -> str | None:
    tile_x, tile_y, px, py = slippy_pixel(lon, lat)
    url = f"{WDIST_BASE}/{target['basetime']}/none/{target['validtime']}/surf/{element}/{TILE_ZOOM}/{tile_x}/{tile_y}.png"
    ...
    for dx, dy in [(0, 0), (1, 0), (-1, 0), (0, 1), (0, -1), (2, 0), (-2, 0), (0, 2), (0, -2)]:
        ...
        if a >= 80:
            labels.append(closest_palette((r, g, b), palette))
    return max(set(labels), key=labels.count) if labels else None


def mesh_forecast(session: requests.Session, targets: list[dict[str, Any]], day: str, start_time: str, lon: float, lat: float) -> dict[str, Any] | None:
    ...
    if not label:
        return None
    temp_band = sample_tile(session, target, "temp", lon, lat, TEMP_PALETTE) if "temp" in (target.get("elements") or []) else None
    valid = utc_target_datetime(str(target.get("validtime") or ""))
    return {"label": label, "meshTempBand": temp_band, "meshTime": valid.strftime("%H:%M"), "validAt": valid.isoformat(timespec="minutes")}


def stable_payload(payload: dict[str, Any], old: dict[str, Any] | None) -> dict[str, Any]:
    if old and old.get("date") == payload.get("date") and old.get("entries") == payload.get("entries") and old.get("failures") == payload.get("failures"):
...
def main() -> int:
    ...
    events = (json.loads(EVENTS_PATH.read_text(encoding="utf-8")).get("events") or [])
    venues = (json.loads(VENUES_PATH.read_text(encoding="utf-8")).get("venues") or [])
    ...
    if COORDS_PATH.exists():
        try:
            ...
            pass
    old_coords = json.dumps(coords_cache.get("venues", {}), ensure_ascii=False, sort_keys=True)

    session = requests.Session()
    session.headers.update({"User-Agent": "KeioKawaiiLabWeather/1.0 (+https://keio-kawaiilab.github.io/keio-kawaii-lab/)"})
    area_map = get_json(session, AREA_URL)
    forecast_area = get_json(session, FORECAST_AREA_URL)
    targets = get_json(session, WDIST_TARGETS_URL)
    ...
    for row in rows:
        ...
        item = grouped.setdefault(key, {**row, "eventIds": []})
        if row["eventId"] and row["eventId"] not in item["eventIds"]:
            item["eventIds"].append(row["eventId"])
    ...
    for key in sorted(grouped):
        ...
        try:
            if office_code not in office_cache:
                office_cache[office_code] = get_json(session, f"{FORECAST_BASE}/{office_code}.json")
            broad = broad_forecast(office_cache[office_code], row["date"], office_code, class10_code, point_code, row["startTime"])
        ...
        mesh = None
        if (date.fromisoformat(row["date"]) - today).days <= MESH_DAYS and geo:
            try:
                mesh = mesh_forecast(session, targets if isinstance(targets, list) else [], row["date"], row["startTime"], float(geo["lon"]), float(geo["lat"]))
            except Exception:
                mesh = None
        ...
        entries.append({
            ...
            "areaName": broad.get("forecastAreaName") or class10_name(class10_code, area_map),
            "precision": "mesh5km" if mesh else "area",
    ...
    if OUTPUT_PATH.exists():
        try:
            ...
            pass
    payload = stable_payload(payload, old_payload)
    OUTPUT_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    new_coords = json.dumps(coords_cache.get("venues", {}), ensure_ascii=False, sort_keys=True)
    ...
    print(f"Event weather refreshed: {len(entries)}/{len(grouped)} venue-date entries, mesh={payload['stats']['meshEntries']}, failures={len(failures)}")
