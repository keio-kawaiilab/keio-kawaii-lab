...
- 確認資料: HOT STUFF `py28674` / `py28675` / `py28311` / `py28309` / `py28310`、ぴあ `eventCd=2634021&rlsCd=001`（昼夜とも予定枚数終了）、`2635331&rlsCd=001`（横浜・当日引換券）、`2628747&rlsCd=001/002`（有明2日）。全5公演とも9/12 10:00開始。横浜締切9/22 23:59、有明締切9/23 23:59。お花見会の終了時刻は未掲載のため推測しない。
- `data/verified-general-sales.json` と `scripts/verified_general_sales.py` で確認済み受付を公演日・開演時刻・グループ・公演名に照合し公開モデルへ補完。お花見会の旧「2公演化」より後で適用するため一般発売が再び消えない。既存の公演を追加・複製しない。新しい販売観測は保持する。
- `install_performance_public_view.py` は正規公開JSON・静的snapshotの両経路で同じ補完を実施。`fix_missing_application_start_ui.py` は予定枚数終了を静的/ブラウザ双方で表示し、販売中CTAと区別。
...
## 2026-09-14 終了受付の申込欄からの除外（修正・検証・本番確認済み）
- 利用者の9/29 CUTIE STREET有明の画像で終了済みFC・ぴあ先行が申込欄に残ることを確認。従来はCTAの文言変更のみで枠を残していた。
- 終了日時を過ぎた受付と予定枚数終了の受付は、静的HTML・ブラウザー描画双方でticket-option全体を出力しない。受付中・受付予定は表示し、公演と履歴データは維持する。
- 本番と同じ再生成を実施し、期限切れ枠の非表示、未来受付の表示、一般販売の維持、公演を消さない既存回帰を検証。

- 本番確認: `d9d748760a4f32998e13722d6fcb235326cc7923`、Schedule shell CI `34791842034` と Pages `34791840886` 成功。公開ブラウザーで9/29有明の終了先行3枠がなく、一般発売1枠・受付中・締切9/23 23:59・申込リンクを確認。

## 2026-09-14 横浜一般販売の履歴欠落（修正・検証・本番確認済み）
- 前回「履歴を維持」としたが、横浜当日引換券は公開モデルにのみ存在し、履歴archiveがraw eventsしか読まないため未保存だった。表示中ticket-optionからの補完がなくなると履歴が欠けた。
- archiveへpublicEvents内のraw未収録offersの保存を追加。出典URLと検証済み日時を引き継ぎ、sold_out状態も永続化。表示側も予定枚数終了と表示する。終了受付は申込欄へ戻さない。
- 既存履歴のIDを再観測なしに書き換える処理も止め、失敗していたappend-onlyテストを復旧。履歴1053件（7件補完）を生成。
- 14件の既存・新規テストと、申込枠がなくても横浜一般販売・根拠URL・期間・予定枚数終了を描画するブラウザー処理の回帰を検証。

- 本番確認: 履歴保存 `72601e9`、履歴JSの直接バージョン指定読込 `757da03`。Ticket flow release check `34792506647` と再生成後Pages `34792671743` 成功。公開画面で横浜の履歴タブを開き「チケットぴあ｜一般発売（当日引換券）」「予定枚数終了」「9/12 10:00〜9/22 23:59」と出典リンクを確認。終了受付は申込欄に出ていない。前回の履歴維持確認不足を訂正。

## 2026-09-14 予定枚数終了の受付帯（修正・検証・本番確認済み）
- 申込枠と履歴の状態を直した一方、カレンダー帯のeffectiveBandは締切時刻しか見ずsold_outを除外していなかった。
- 最終生成処理で予定枚数終了の受付は帯を返さないよう修正。受付可能な帯、公演、保存済み履歴を維持。横浜一般販売および将来の受付期間でもsold_outなら帯なしとなる回帰を追加。

- 本番確認: 修正 `78d4c75`、再生成後Pages `34795666522` 成功。公開カレンダーのCUTIE STREET表示で横浜当日引換券の帯なし、有明の受付中一般発売の帯あり、横浜公演と履歴の「予定枚数終了」維持を確認。

## 2026-09-14 交通情報の対象公演表示（修正・検証・本番確認済み）
- トップの交通情報が路線名だけで対象公演不明。路線別に当日の公演名（グループ含む）、日付、開演時刻、会場を表示する。
- summaryはcanonical publicEventsを優先。同日・同じ会場に関係する路線のみを照合し、同じ公演の重複を除外。別日の公演を表示しない回帰を追加。JS/CSSの読込URLを更新し再生成にも引き継ぐ。

- 本番確認: `115eab6`、再生成後Pages `34797558137` 成功。公開通知で「CUTIE STREET 梅田みゆ 生誕祭 2026」「9/14 開演 19:00 ／ SGCホール有明」「りんかい線 運転状況」を確認。既存路線照合・同一公演重複除外・別日除外テスト成功。
